...
Usage:
    ...
    python3 clean_node.py ahab1101 --non-interactive
...
def confirm(prompt, non_interactive=False):
    if non_interactive:
        print(f"{prompt} [y/N]: y (auto-approved, --non-interactive)")
        return True
...
def process_namespace(namespace, pods, target_node, dry_run, timeout, poll_interval, non_interactive):
    ...
    for sel_key, group_pods in selector_groups.items():
        ...
        if batch_mode:
            ...
            if dry_run:
                ...
                continue
            if not confirm(f"  Delete {len(pod_names)} pod(s) in {label} as a batch?", non_interactive):
            ...
            for pod in group_pods:
                ...
                if dry_run:
                    ...
                    continue
                if not confirm(f"  Delete pod {pod_name}?", non_interactive):
                    print("  Skipped by user.")
                    continue
...
def main():
    ...
    parser.add_argument("--non-interactive", action="store_true",
                         help="Skip all y/N prompts (initial proceed + per-pod/batch confirmations). "
                              "All other logic (cordon check, exclusions, readiness waits, abort-on-timeout) "
    ...
    if not args.dry_run:
        if not confirm(f"\nProceed with migrating pods off {target_node}?", args.non_interactive):
    ...
    for ns, ns_pods in sorted(grouped.items()):
        if ns in EXCLUDED_NAMESPACES:
            ...
            continue
        process_namespace(ns, ns_pods, target_node, args.dry_run, args.timeout, args.poll_interval, args.non_interactive)
